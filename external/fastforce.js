d3.layout.fastForce = function() {
  ...
  function approxRepulse(node) {
    ...
    randIndices = getRandIndices(indicesRepulse, Math.sqrt(n));
    ...
    for (i = randIndices.length - 1; i >= 0; --i) {
      ...
      if (node.nearest.indexOf(currNode) >= 0) continue;

      dx = currNode.x - node.x;
      dy = currNode.y - node.y;
      dn = dx * dx + dy * dy;

      if (dn && dn < chargeDistance2) {
        k = (alpha * charges[currNode.index] * randIndices.length) / dn;
    ...
    for (i = nearest.length - 1; i >= 0; --i) {
      ...
      if (dn && dn < chargeDistance2) {
        k = (alpha * charges[currNode.index]) / dn;
  ...
  force.tick = function() {
    ...
    // adjusted alpha to converge better, but stop after same number of iterations
    if ((alpha *= 0.98) < 0.00024) {
      timer = null;
      event.end({type: "end", alpha: alpha = 0, mean: d3.mean(times)});
      return true;
    }
    ...
    // apply charge forces to a random selection of nodes
    if (charge) {
      // choose random sqrt(n) nodes to compute forces
      randIndices = getRandIndices(indicesForce, n);
      min = Math.sqrt(n);
      i = -1; while (++i < min) {
        // compute forces on these sqrt(n) nodes
        approxRepulse(nodes[randIndices[i]]);
      }

      // use vector from previous update for remaining nodes
      --i; while (++i < n) {
        o = nodes[randIndices[i]];
        constantRepulse(o);
        o.px -= o.vx;
        o.py -= o.vy;
      }
  ...
  force.start = function() {
    ...
    // Initialize arrays
    indicesForce = d3.range(0, n);
    indicesRepulse = d3.range(0, n);
    ...
    for (i = 0; i < n; ++i) {
      ...
      o.nearest = getRandIndices(indicesRepulse, Math.sqrt(n));
    }
    ...
    function position(dimension, size) {
      ...
      var candidates = neighbors[i],
          ...
          l = candidates.length,
          x;
      while (++j < l) if (!isNaN(x = candidates[j][dimension])) return x + Math.random() - 0.5;
