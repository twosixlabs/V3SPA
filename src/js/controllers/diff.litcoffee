...
Enumerate the differences between the two policies
      ...
      find_differences = () =>
        ...
        $scope.modules = d3.entries([].reduce(((modules, rule) -> #d3.entries(d3.merge([$scope.rules, comparisonRules]).reduce(((modules, rule) ->
        ...
        linkScale.domain [1,1] #d3.extent(graph.links, (l) -> return l.rules.length)
