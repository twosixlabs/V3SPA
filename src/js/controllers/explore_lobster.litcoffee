    ...
    vespaControllers.controller 'exploreLobsterCtrl', ($scope, VespaLogger, WSUtils, $compile,
      ...
      $scope.update_view = () ->
        ...
        if not $scope.policy?.json?.parameterized?.condensed_lobster?
          return
        ...
        $scope.policy.json.parameterized.condensed_lobster.nodes.forEach (n) ->
          if n.name.indexOf('.') == -1
      ...
      IDEBackend.add_hook "policy_load", IDEBackend.load_lobster_graph
      ...
      if $scope.policy?._id and not $scope.policy.json?.parameterized?.condensed_lobster?
        IDEBackend.load_lobster_graph()

      # If the graph data is already loaded, render the view
      if $scope.policy?.json?.parameterized?.condensed_lobster?
        $scope.update_view()